...
const Clases = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setMensaje('Error al obtener los libros y autores.');
      }
    };
    fetchData();
  }, []);
  ...
  const validateForm = () => {
    ...
    if (clases.some(clase => clase.horario === formData.horario && clase.id !== formData.id)) {
      newErrors.horario = 'Ya existe un libro con este nombre';
    }

    // Verificar campos vacíos
    if (!formData.nombre) newErrors.nombre = 'El titulo es requerido';
    if (!formData.horario) newErrors.horario = 'El Año es requerido';
    if (!formData.idEntrenador) newErrors.idEntrenador = 'Seleccione un autor';
    ...
    if (validateForm()) {
      try {
        ...
        if (editando) {
          ...
          console.log("libro actualizada:", claseActualizada);
          ...
          console.log("Nuevo libro creada:", nuevaClase);
        ...
        console.error('Error al crear o actualizar libro:', error);
        setMensaje('Error al crear o actualizar el libro. Intenta nuevamente.');
      }
    } else {
        ...
        <div className="grid lg:grid-cols-2 gap-16">
          ...
          <div className="bg-white shadow-xl rounded-lg p-8 space-y-8">
            <h2 className="text-3xl font-semibold mb-8 flex items-center space-x-4">
              ...
              <span>{editando ? 'Editar libro' : 'Agregar libro'}</span>
            </h2>
            <form onSubmit={handleSubmit}>
              ...
              >
                {editando ? 'Actualizar libro' : 'Crear libro'}
              </button>
            </form>
          </div>
          ...
          {/* Lista de Clases */}
          <div className="bg-white shadow-xl rounded-lg p-8 space-y-8">
            <h2 className="text-3xl font-semibold mb-8">Libros registrados</h2>
            {clases.length > 0 ? (
